...
# Partie specifique a la version cloud-init
...
#export RAM_VM=8192                  # Decommenter pour donner a la VM une quantite de ram specifique
...
cloud-localds ${TMP}/cloud.img cloud-config.yaml
...
qemu-system-x86_64 \
    ...
    -net nic \
    -net user \
    -hda ${TMP}/${VERSION}-server-cloudimg-amd64.img \
    -hdb ${TMP}/cloud.img \
    -nographic
...
rm -f ${TMP}/${VERSION}-server-clouding-amd64.img ${TMP}/cloud.img
...
## PARTIE II : INSTALLATION DES PAQUETS POUR GENERER L'ISO ET CONFIGURATION DU CHROOT
echo "Partie II: installation des paquets pour generer l'iso et configuration du chroot"
...
chroot_prepare
cp ${SCRIPTS}/chrooted_cloud.sh ${CHROOT}/root/chrooted_cloud.sh
chmod +x ${CHROOT}/root/chrooted_cloud.sh

# Execution du script dans le chroot
chroot ${CHROOT} /root/chrooted_cloud.sh

chroot_after
rm ${CHROOT}/root/chrooted_cloud.sh
